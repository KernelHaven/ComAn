 ...
 *
 */
public class DiffAnalyzerBusyboxTests extends AbstractTests {

	/**
	 * The {@link File} defining the directory where the commits are stored.
	 */
	private static final File TEST_INPUT_DIR = new File("testdata/test_input_busybox");
	 ...
	 */
	private static final File EXPECTED_VALUES_FILE = new File(TEST_INPUT_DIR, "busybox_ExpectedValues.txt");
		...
		if (expectedValuesLines != null && !expectedValuesLines.isEmpty()) {
			...
			for (File testCommit : testCommits) {
				...
				if (expectedValuesVector == null) {
					ComAnLogger.getInstance().log("DiffAnalyzerBusyboxTests", "Loading expected values for \"" + testCommitNumber + "\" failed",
	 ...
	 * in {@link #EXPECTED_VALUES_FILE} for each commit of the busybox-repository.
	 */
	@Test
	public void testCommits() {
		for (File testCommit : testCommits) {
			...
			this.compareResultsToExpectedValues("DiffAnalyzerBusyboxTests", TEST_INPUT_DIR, testCommit.getName(),
