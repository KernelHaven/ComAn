...
public class DiffAnalyzerLinuxTests extends AbstractTests {
		...
		if (expectedValuesLines != null && !expectedValuesLines.isEmpty()) {
			...
			for (File testCommit : testCommits) {
				...
				if (expectedValuesVector == null) {
					ComAnLogger.getInstance().log("DiffAnalyzerLinuxTests", "Loading expected values for \"" + testCommitNumber + "\" failed",
	 ...
	 * in {@link #EXPECTED_VALUES_FILE} for each commit of the linux kernel-repository.
	 */
	@Test
